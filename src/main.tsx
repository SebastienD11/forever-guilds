...
function PlanForm({ guildId, guildName, onDone, onClose }: { guildId: string; guildName: string; onDone: () => void; onClose: () => void }) {
  ...
  return <form onSubmit={submit} className="modal-form"><div className="modal-intro"><span className="eyebrow gold">A NEW CHAPTER</span><h2>Share a reunion plan.</h2><p>Tell old guildmates where you intend to gather in Forever.</p></div>
...
function App() {
  ...
  return <Cursor faction="human" className="site-shell"><header className="site-header"><div className="header-inner"><a className="brand" href="/" onClick={e => { e.preventDefault(); closeDetail(); }}><span className="brand-mark"><Shield size={22} /><span>✦</span></span><span><strong>FOREVER</strong><em>GUILDS</em></span></a><nav className="nav"><span><Button className="header-primary" onClick={() => setModal("guild")}><Plus size={14} /> Add your guild</Button></span></nav></div></header>
  <main>{detail ? <Detail detail={detail} onBack={closeDetail} onPlan={() => setModal("plan")} onMemory={() => setModal("memory")} /> : <><section className="hero"><div className="hero-glow" /><div className="hero-inner"><div className="hero-copy"><h1><span>Some bonds are</span><br /><i>forever.</i></h1><p>Remember the guild that made Azeroth feel like home? Find your old comrades, share a memory, and see where your paths might meet again in World of Warcraft: Forever.</p><div className="hero-actions"><Button onClick={() => document.getElementById("archive")?.scrollIntoView({ behavior: "smooth" })}>Find your guild <ArrowRight size={17} /></Button><Button variant="frame" className="hero-secondary-button" onClick={() => setModal("guild")}><Plus size={17} /> Add an old guild</Button></div></div><div className="hero-art" aria-hidden="true"><div className="outer-ring"><div className="inner-ring"><div className="world"><div className="mountain mountain-back" /><div className="mountain mountain-front" /><div className="portal"><div className="portal-core" /></div></div></div></div><span className="rune rune-one">✦</span><span className="rune rune-two">✧</span><span className="rune rune-three">✦</span></div></div></section>
